...
private:
  ...
  struct InnerNode : public node {
    ...
    unsigned int subtsizes[inner_slotmax+1];
...
private:
  ...
  std::pair<iterator, bool> insert_start(const key_type &key,
    ...
    if (newchild) {
      ...
      newroot->subtsizes[0]=root_->subtsize;
      newroot->subtsizes[1]=newchild->subtsize;
  ...
  std::pair<iterator, bool> insert_descend(node *n, const key_type &key,
    ...
    if (!n->is_leafnode()) {
      ...
      std::pair<iterator, bool> r =
          ...
          inner->subtsizes[slot]=inner->childid[slot]->subtsize;
      if (newchild) {
        ...
        if (inner->is_full()) {
          ...
          if (slot == inner->slotuse + 1 &&
            ...
            inner->subtsizes[inner->slotuse+1]=tp;
            inner->slotuse++;
            inner->subtsize+=tp;
            ...
            split->subtsizes[0]=newchild->subtsize;
        ...
        std::copy_backward(inner->subtsizes + slot,
                           inner->subtsizes + inner->slotuse + 1,
                           inner->subtsizes + inner->slotuse + 2);                   

        inner->slotkey[slot] = newkey;
        inner->childid[slot + 1] = newchild;
        inner->subtsizes[slot+1]=newchild->subtsize;
        inner->slotuse++;
        inner->update_subtsize();
      }
      else{
  ...
  void split_inner_node(InnerNode *inner, key_type *out_newkey,
    ...
    std::copy(inner->childid + mid + 1, inner->childid + inner->slotuse + 1,
              newinner->childid);
    std::copy(inner->subtsizes + mid + 1, inner->subtsizes + inner->slotuse + 1,
              newinner->subtsizes);
    inner->slotuse = mid;
    newinner->update_subtsize();

    inner->update_subtsize();
    //inner->subtsize-=newinner->subtsize;//can improved -=newinner->
  ...
  static void print_node(std::ostream &os, const node *node,
    ...
    if (node->is_leafnode()) {
      ...
      for (unsigned int i = 0; i < depth; i++)
        os << "  ";
      os<<"sizes of son:";
      for(unsigned int i=0;i<= innernode->slotuse;++i){
        os<<innernode->subtsizes[i]<<"  ";
      }
      os<<std::endl;
      if (recursive) {
        for (unsigned short slot = 0; slot < innernode->slotuse + 1; ++slot) {
  ...
  void verify_node(const node *n, key_type *minkey, key_type *maxkey,
    ...
    } else // !n->is_leafnode()
    {
      ...
      for(int i=0;i<=inner->slotuse;++i){
        ...
        tlx_die_unless(inner->childid[i]->subtsize==inner->subtsizes[i]);
      }
